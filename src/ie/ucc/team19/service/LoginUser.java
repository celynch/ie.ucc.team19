...
public class LoginUser {
    ...
    public LoginUser(DBConnectionManager connector) {
        this.connector = connector;
    }

    private StudentBean loginStudent(String email, String password) {
        StudentBean student = new StudentBean();
        String query = "SELECT * FROM students WHERE email = ?";
        Object[] params = {email};
        ArrayList<Map<String, String[]>> studentDetails = connector.Select(query, params);
        if(studentDetails.size() != 0) {// user exists
            if( studentDetails.get(0).get("password")[0].equals(password)) { // password matches
                try {
    ...
    private AdminBean loginAdmin(String adminName, String adminPassword) {
        ...
        ArrayList<Map<String, String[]>> adminDetails = connector.Select(query, params);
    ...
    public StudentBean cookieValidate(String email, String cookie_token) {
        StudentBean student = new StudentBean();
        String query = "SELECT * FROM students WHERE email = ?";
        Object[] params = {email};
        ArrayList<Map<String, String[]>> studentDetails = connector.Select(query, params);
        if(studentDetails.size() != 0) {    // email found
            if( studentDetails.get(0).get("cookieToken")[0].equals(cookie_token)) { // cookieToken matches
                try {
                    ...
                    UpdateUser updater = new UpdateUser(connector);
    ...
    public boolean loginViaForm(HttpServletRequest request, HttpServletResponse response) {
        ...
        StudentBean student = loginStudent(email, password);
    ...
    public void loginViaCookie(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(cookies != null) {
            ...
            StudentBean student = cookieValidate(email, cookie_token);
            if(student.getEmail() != null && student.isAuthenticated()) {
                request.getSession().setAttribute("user", student);
                setCookies(response, student, request.getServerName(),
                        Boolean.parseBoolean(request.getParameter("rememberMe")), false);
    ...
    public void loginVerify(HttpServletRequest request, HttpServletResponse response) {
        StudentBean student = loginStudent(request.getParameter("email"), request.getParameter("password"));
